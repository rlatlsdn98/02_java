package com.qew032.section02.looping.level03.hard;
...
public class Application3 {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < input.length(); i++){
            int opnum = input.charAt(i);

            if(!((opnum >= 'a' && opnum <= 'z') || (opnum >= 'A' && opnum <= 'Z'))) {
                System.out.println("영문자가 아닌 문자가 포함되어 있습니다.");

                return;
            } else if(input.charAt(i) == op){
                count++;
            }

        }

        System.out.println("포함된" + op + " 의 갯수 = " + count);
