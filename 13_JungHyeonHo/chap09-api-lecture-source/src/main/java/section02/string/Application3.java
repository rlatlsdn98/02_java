...
public class Application3 {
  public static void main(String[] args) {
    ...
    String[] empArr3 = emp3.split("/"); // 마지막 /이후 값이 없으면 무시한다, 맨처음 /는 인정이요
    ...
    String[] empArr5 = emp3.split("/", 3); // 3번째 / 전까지는 구분해주고 이후는 한번에 출력

    for(int i = 0; i < empArr4.length; i++) {
      System.out.println("empArr4[" + i + "] : " + empArr4[i]);		//마지막 / 도 인식해줌
    }

    for(int i = 0; i < empArr5.length; i++) {
      System.out.println("empArr5[" + i + "] : " + empArr5[i]);		//2번째 /까지
    }
    /* !!!! 반면 StringTokenizer의 경우 공백으로 존재하는 값을 무시해버린다. */
    StringTokenizer st1 = new StringTokenizer(emp1, "/"); // delimeter : 구분해준다
    StringTokenizer st2 = new StringTokenizer(emp2, "/");
    StringTokenizer st3 = new StringTokenizer(emp3, "/");

    while(st1.hasMoreTokens()) { //있는지 먼저 무조건 확인해야됨. 꺼내려고했는데 없으면 오류나용~
    ...
    /* 그리고, 1개씩만 꺼내볼 수 있다! Stack구조로 잘라서 꺼내먹어용 */
...
//		String[] colors = colorStr.split("*#/ ");		//에러남, \\ 붙이면 해줌
