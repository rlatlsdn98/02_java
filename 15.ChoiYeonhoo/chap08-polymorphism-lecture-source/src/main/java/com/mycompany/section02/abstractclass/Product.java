...
public abstract class Product {
  ...
  /* 필드 */
  // 추상 클래스는 일반적인 필드를 가질수 있다.
  private int nonStaticField; // 인스턴스 변수 (인스턴스 생성시 만들어지는 변수,스태틱이 아닌 변수)
  private static int staticField; // 클래스 변수

  /* 메서드 */
  // 생성자
  // - 추상 클래스는 직접 인스턴스를 생성할 수 없지만,
  //  상속 받은 자식내 부모 부분 생성을 위해서 필요하다.
  public Product(){}
  ...
  * -> 추상 메서드임을 명시하기 위해 abstract keyword를 반드시 붙여야 한다.
  *
  * - 추상 메서드가 존재하는 클래스에는
  *   반드시 abstract keyword가 붙어야한다! (abstract class)
  *
